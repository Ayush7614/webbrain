...
function checkedSyncPassword() { const password = profileSyncPassword?.value || ''; if (password.length < 12) throw new Error('Use a sync password of at least 12 characters.'); if (profileSyncConfirm?.value && password !== profileSyncConfirm.value) throw new Error('Sync passwords do not match.'); return password; }
...
document.getElementById('btn-profile-sync-enable')?.addEventListener('click', () => { try { profileSyncAction('profile_sync_unlock', { password: checkedSyncPassword(), create: true }); } catch (e) { showProfileSyncResult(false, e.message); } });
...
document.getElementById('btn-profile-sync-change')?.addEventListener('click', () => { const oldPassword = window.prompt('Current sync password:'); if (!oldPassword) return; try { profileSyncAction('profile_sync_change_password', { oldPassword, newPassword: checkedSyncPassword() }); } catch (e) { showProfileSyncResult(false, e.message); } });
...
document.getElementById('btn-profile-sync-reset')?.addEventListener('click', () => { if (!window.confirm('Permanently delete the encrypted cloud vault and replace it with this device’s local data?')) return; try { profileSyncAction('profile_sync_reset', { password: checkedSyncPassword() }); } catch (e) { showProfileSyncResult(false, e.message); } });
