...
async function handleMessage(msg, sender) {
  ...
  switch (msg.action) {
    ...
    case 'profile_sync_unlock': await browser.storage.local.set({ [PROFILE_SYNC_KEYS.enabled]: true }); return { ok: true, ...(await profileSync.unlock(String(msg.password || ''), !!msg.create)) };
    case 'profile_sync_now': return { ok: true, ...(await profileSync.sync()) };
