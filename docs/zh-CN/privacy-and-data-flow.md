...
- **云提供商**：OpenAI、Anthropic、Google Gemini、Mistral、DeepSeek、xAI、Groq、OpenRouter 等——数据会离开用户的机器
- **本地提供商**：llama.cpp、Ollama、LM Studio、Jan、vLLM、SGLang——数据保留在用户的机器上

扩展程序本身永远不会在任何远程服务器上接收或存储用户数据。
...
存储在 `chrome.storage.session`（Chrome）或内存中（Firefox）。用于在 Service Worker 重启后恢复对话。永远不会被传输。

### 追踪记录器

“记录追踪”默认启用，可在设置 → 显示中关闭。启用期间，每次代理运行都会写入一个 IndexedDB 数据库（`webbrain_traces`）：
