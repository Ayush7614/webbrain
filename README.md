...
- The OTP / verification-code helper also ships enabled by default and loads only for relevant requests. It declares no network tool: on the active run tab, it prefers selected text or a bounded accessibility-tree subtree, matches the newest relevant service code, excludes SMS/native-app access, and honors Strict secret handling. When used, the scoped page content and code are included in the normal request to your configured LLM provider. Record traces is enabled by default, so raw tool results and model responses are also stored locally until those traces are deleted; disable Record traces in Settings if you do not want that local history. Remove the skill from Settings → Skills if you do not want this guidance available.
